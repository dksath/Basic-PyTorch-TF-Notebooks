...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            # increment gradient in training example for all classes 
            dW[:, j] += X[i] * norm_exp_f[j]
            
            
        # subtract value from ground-truth class in training example
        dW[:, y[i]] -= X[i]
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    loss_matrix = np.exp(scores)/sum_exp_scores
    loss = np.sum(-np.log(loss_matrix[np.arange(num_train), y]) )
